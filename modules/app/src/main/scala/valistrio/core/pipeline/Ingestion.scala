...
/** Orchestrates the /post flow: parse, validate (reusing [[Validation]]), then either write the
  ...
  * Valistrio owns, salvage it to the DLQ sink.
  *
  ...
  * sink's responsibility for 0.1.0 — Ingestion does not check for duplicates.
  */
class Ingestion(
  ...
  def ingest(rawBody: String): IO[PostResponse] =
